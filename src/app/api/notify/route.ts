...
export async function POST(request: Request) {
  try {
    ...
    // Send confirmation preview to admin (using the customer template)
    const customerSubject = `Customer Confirmation Preview - ${businessName}`;
    const isFreeTier = tier === 'free';
    
    let customerHtml = '';
    const contactDisplay = `<p><strong>Customer Contact:</strong> ${contactInfo}</p>`;
    ...
    if (isFreeTier) {
      customerHtml = `
        <div style="font-family: sans-serif; line-height: 1.6; color: #333;">
          ...
          <hr />
          <p>Hey ${name}!</p>
          <p>I&apos;ve got your request for <strong>${businessName}</strong> and I&apos;m excited to build your site!</p>
